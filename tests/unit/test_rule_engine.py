...
from rules.rule_engine import StandardMoveValidator


def test_validate_move_out_of_bounds():
    board = Board(8, 8)
    validator = StandardMoveValidator()
    assert not validator.validate(board, Position(0,0), Position(8,8)).is_valid


def test_validate_move_empty_source():
    board = Board(8, 8)
    validator = StandardMoveValidator()
    assert not validator.validate(board, Position(1,1), Position(2,2)).is_valid
...
def test_validate_move_friendly_fire():
    ...
    validator = StandardMoveValidator()
    res = validator.validate(board, Position(0,0), Position(0,1))
